...
class Decoder(nn.Module):
    def __init__(self, mdim):
        ...
        self.pred_Q = nn.Conv2d(mdim, 1, kernel_size=(3, 3), padding=(1, 1), stride=1)
    ...
    def forward(self, r4, r3, r2):
        ...
        Q_propagate2 = F.interpolate(self.pred_Q(m2), scale_factor=0.25, mode='nearest')
        Q_propagate3 = F.interpolate(self.pred_Q(m3), scale_factor=0.5, mode='nearest')
        Q_propagate4 = F.interpolate(self.pred_Q(m4), scale_factor=1, mode='nearest')
        Q_propagate = torch.cat((Q_propagate2 * 1, Q_propagate3 * 2, Q_propagate4 * 4), 1)  # len:batchsize
        Q_propagate = torch.sum(Q_propagate, dim=1, keepdim=True)
        return p,Q_propagate  # , p2, p3, p4
...
class Memory(nn.Module):
    ...
    def forward(self, m_in, m_out, q_in, q_out):  # m_in: o,c,t,h,w
        ...
        #caculate sim, pdt:production
        vec_pdt = torch.sum(torch.mul(m_in[:,:,0],q_in),dim=(1,2,3))
        # sca_pdt = torch.norm(mi,dim=(1,2))*torch.norm(qi,dim=(1,2))#1.4.0及以上版本不适用
        sca_pdt = torch.zeros_like(vec_pdt)
        batch_size = vec_pdt.shape
        for bz in range(batch_size[0]):
            sca_pdt[bz] = torch.norm(mi[bz])*torch.norm(qi[bz])
        # sim = torch.exp(vec_pdt / sca_pdt)# 我个人认为带了exp以后就没有归一化一说了
        sim = vec_pdt / sca_pdt
        ...
        return mem_out, p, sim# mem_out是通过相似度函数找出来的value和Query value的结合体，P是两个key的相似度函数
...
class EMN(nn.Module):
    ...
    # def Soft_aggregation(self, ps, K):
    #     num_obj, H, W = ps.shape
    #     em = ToCuda(torch.zeros(1, K, H, W))
    #     em[:, 0] = torch.prod(1 - ps, dim=0)  # bg prob 背景就是所有层里没有object的位置
    #     # em[:, 0] = 1 - ps  # bg prob 背景就是所有层里没有object的位置
    #     em[:, 1:num_obj + 1] = ps  # obj prob
    #     # em[:, 1] = ps  # obj prob
    #     em = torch.clamp(em, 1e-7, 1 - 1e-7)
    #     logit = torch.log((em / (1 - em)))
    #     return logit
    ...
    def segment(self, frame, keys, values, num_objects):
        ...
        m4, viz, sim = self.Memory(keys[:, 1], values[:, 1], k4e, v4e)
        m4_new = self.ASPP(m4)
        logits,Q_propagate = self.Decoder(m4_new, r3e, r2e)  # 有skip connection
